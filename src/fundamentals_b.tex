...
Instead, we only extract the ingredients required for the later discussion of the dielectric function, the Kramers--Kronig relations, and the optical properties~\cite{landau2013electrodynamics}.
...
and it also provides the starting point for introducing the dielectric function\cite{landau2013electrodynamics, bernasconi2011first}.
...
Therefore, it is convenient to introduce the in-plane dielectric function $\varepsilon_{\varparallel}(\omega)$ and the out-of-plane dielectric function $\varepsilon_{\perp}(\omega)$~\cite{nye1985physical}:
...
which is the frequency-dependent dielectric function~\cite{wooten2013optical, onida2002electronic}.
...
This complex dielectric function therefore provides a unified framework for the linear optical properties~\cite{djurivsic2002progress}.

These two parts of the dielectric function together determine the optical response~\cite{wooten2013optical, born2013principles,dressel2002electrodynamics,fox2010optical}.
...
Therefore, the complex dielectric function provides the complete information required for the subsequent optical quantities~\cite{vandenberghe2023two}.
...
which characterizes the dominant energy dissipation channels.

In what follows, we present the general relations that
connect these quantities to the complex dielectric function $\varepsilon(\omega)$~\cite{wooten2013optical,born2013principles,dressel2002electrodynamics,fox2010optical}.
...
The following figure presents the calculated absorption coefficient spectra of the present systems for the in-plane and out-of-plane directions. % cite my third project here
...
It also provides a direct measure of directional optical attenuation~\cite{zhang2024advanced}.

Consequently, the absorption coefficient characterizes the rate of attenuation of incident light in the medium.
We next turn to the complex refractive index and determine the refractive index and extinction coefficient.
...
We now continue this description of light propagation in the medium by introducing the refractive index $n_{\lambda}(\omega)$ and extinction coefficient $k_{\lambda}(\omega)$.
...
we derive explicit expressions for $n_{\lambda}(\omega)$ and $k_{\lambda}(\omega)$~\cite{wooten2013optical,born2013principles,cheng2025two,kim2024hyperspectral}.

For a monochromatic wave propagating along the principal direction $\lambda$,
we first represent the electric field as~\cite{fox2010optical,born2013principles,politano2023spectroscopic}:
...
We then write the general relation between the complex refractive index $N_{\lambda}(\omega)$, dielectric function $\varepsilon_{\lambda\lambda}(\omega)$, and magnetic permeability $\mu_{\lambda\lambda}(\omega)$ as~\cite{wooten2013optical,dressel2002electrodynamics,politano2023spectroscopic}:
...
and the definition of the complex refractive index in equation~\eqref{complex_refractive_index_for_absorption_coefficient}
into equation~\eqref{nonmagnetic_refractive_index_dielectric_relation},
and then separate the real and imaginary parts.
We therefore derive the coupled relations~\cite{wooten2013optical,dressel2002electrodynamics,fox2010optical,cheng2025two,kim2024hyperspectral}:
...
With these expressions, we present the calculated refractive index spectra of $\alpha$-Beryllene and $\beta$-Beryllene~\cite{tang2025unveiling,huang2023optical,suk2024ultrafast,cheng2025two}: % cite my third project
...
These trends indicate that the phase response depends on both the optical direction and the crystal structure of the medium~\cite{huang2023optical,cheng2025two}.

We next present the extinction coefficient spectra,
which follows the absorptive part of the dielectric response more directly~\cite{wooten2013optical,dressel2002electrodynamics,politano2023spectroscopic}:
...
This behavior shows that amplitude attenuation also depends on both the optical direction and the crystal structure of the medium~\cite{suk2024ultrafast,cheng2025two,tang2025unveiling}.
...
Meanwhile, they connect the dielectric function to the interface response through the Fresnel equations~\cite{wooten2013optical,dressel2002electrodynamics,politano2023spectroscopic}.
We next use them to determine the reflectivity.
...
To describe this surface response, we introduce the reflectivity $R_{\lambda}(\omega)$.
...
With the complex refractive index in equation~\eqref{complex_refractive_index_for_absorption_coefficient},
...
It equals the squared modulus of the amplitude reflection coefficient~\cite{born2013principles,dressel2002electrodynamics}:
...
this equation produces the reflectivity through the refractive index $n_\lambda(\omega)$ and the extinction coefficient as $k_\lambda(\omega)$:
...
The reflectivity spectra demonstrate a similar directional contrast and closely follow the behavior of the extinction coefficient.
...
This behavior indicates that surface reflection is strongly influenced by optical dissipation, while it is further modulated by the refractive index.

In the in-plane channel, $\beta$-Beryllene exhibits strong reflection in the low-energy and near-ultraviolet regions with a pronounced minimum in between,
whereas $\alpha$-Beryllene varies more smoothly.
In the out-of-plane channel, $\beta$-Beryllene remains stronger at low energies and develops oscillatory features at higher photon energies,
while $\alpha$-Beryllene shows pronounced peaks in the ultraviolet region.
This behavior indicates that reflectivity is mainly governed by optical dissipation and further modulated by the refractive index.

%%

Because the reflectivity is determined by $n_{\lambda}(\omega)$ and $k_{\lambda}(\omega)$,
its spectral structures inherit the directional anisotropy already seen in the refractive index and extinction coefficient.
Meanwhile, the peak positions and valleys in $R_{\lambda}(\omega)$ identify the energy windows where the interface mismatch and absorptive response combine most strongly.

Consequently, the reflectivity $R_{\lambda}(\omega)$ characterizes the surface return of incident light in the medium.
We arrive at a direct interface observable derived from the complex refractive index.
We next turn to the energy-loss spectrum and describe the collective energy dissipation encoded in the dielectric response.
