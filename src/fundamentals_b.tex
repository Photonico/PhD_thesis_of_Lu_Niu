...
Optical experiments usually probe the response of a material under an oscillating electromagnetic field.
...
Secondly, crystalline materials usually do distinguish directions.
...
At this point, we have clarified that the dielectric function describes how internal charges polarize under an external electric field and how that response changes with frequency.

% Draft



Once we enter the frequency domain, the optical response no longer appears as a static redistribution of charge.
...
For a monochromatic perturbation, the induced polarization does not, in general, follow the applied field instantaneously.
One part of the response stays in phase with the driving field, while another part lags behind it.
...
here, $\varepsilon_{1,\alpha\beta}(\omega)$ is the real part of the dielectric tensor, and $\varepsilon_{2,\alpha\beta}(\omega)$ is the imaginary part of the dielectric tensor.
The real part describes the dispersive part of the response.
In physical terms, it records how the medium builds reversible polarization under the applied field, and therefore how it stores electric-field energy.
The imaginary part describes the absorptive part of the response.
In physical terms, it records how the field opens excitation channels in the electronic system and transfers energy into the medium~\cite{onida2002electronic}.

For the later discussion of optical observables, it proves convenient to suppress the tensor indices once the field polarization has been fixed, or once we work along a principal direction of the dielectric tensor.
The corresponding complex dielectric function then reads:
...
here, $\varepsilon_1(\omega)$ is the real part of the dielectric function, and $\varepsilon_2(\omega)$ is the imaginary part of the dielectric function.
This scalar notation does not discard the tensor character of the response.
Instead, it isolates one resolved channel of that response and prepares the form used in the later derivation of optical quantities.

The complex dielectric function is central not only because it describes the electric response itself, but also because it provides the common starting point for the later optical analysis.
Once $\varepsilon(\omega)$ is known, we can derive the absorption coefficient $\alpha(\omega)$, the complex refractive index $N(\omega)$ together with the refractive index $n(\omega)$ and the extinction coefficient $k(\omega)$, the reflectivity $R(\omega)$, and the energy-loss function $L(\omega)$ from the same response function.
In this sense, the dielectric function does not represent only one observable among many.
It organizes the whole chain of linear optical properties into a unified framework~\cite{gajdos2006linear}.

At the same time, the two parts of the dielectric function do not play independent roles.
In practical first-principles calculations, the imaginary part $\varepsilon_2(\omega)$ is directly tied to electronic excitations, while the real part $\varepsilon_1(\omega)$ follows from the causality structure of the response.
Therefore, once we determine the complex dielectric function, we do not merely obtain a numerical spectrum.
We also recover the physical route by which the applied field drives polarization, transfers energy, and finally shapes the measurable optical response~\cite{onida2002electronic,gajdos2006linear}.

For the layered systems considered in the present work, directional dependence remains essential.
We therefore later resolve the dielectric response into the in-plane and out-of-plane channels and compare them separately.
This directional decomposition is particularly important for anisotropic layered materials, where the dielectric response along the structural plane can differ substantially from that along the perpendicular direction~\cite{marinopoulos2002anisotropy}.
At the present stage, however, the key point is already clear.
The applied field drives a polarization response, the electric susceptibility tensor quantifies that response, and the complex dielectric function collects it into the form most useful for first-principles optical analysis.

Therefore, we arrive at the complex dielectric function as the central response quantity of the present chapter.
What remains is to identify which microscopic electronic processes generate the imaginary part $\varepsilon_2(\omega)$, and how the real part $\varepsilon_1(\omega)$ follows from it.
We now turn to the first of these two questions.
