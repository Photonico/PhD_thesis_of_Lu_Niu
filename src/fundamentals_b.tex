...
% A representative example of the direction-resolved dielectric response is shown in figure~\ref{fig:representative_dielectric_response}.
% The real and imaginary parts exhibit distinct in-plane and out-of-plane features over the full photon-energy range.

% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=0.88\textwidth]{figures/representative_dielectric_response.pdf}
...
%     \label{fig:representative_dielectric_response}
% \end{figure}
...
Consequently, the complex dielectric function describes the absorptive and dispersive responses of the medium within a unified framework.
...
We now examine the static limit and clarify its physical interpretation.

Firstly, the static dielectric response is determined by the zero-frequency limit of the dispersive part.
In the principal-axis representation, we set $\omega\to0$ in equation~\eqref{kk_positive_frequency_dielectric_component} and obtain:
...
Instead, the full directional absorption spectrum $\varepsilon_{2,\lambda\lambda}(\omega)$ determines it through causality.

Next, the factor $1/\omega'$ gives stronger weight to low-energy spectral contributions.
Low-energy electronic transitions therefore enhance the static dielectric response more efficiently, whereas higher-energy transitions contribute with weaker weight.
The static limit thus represents an accumulated response over the full absorption spectrum, with the dominant contribution controlled by its low-energy part.

Meanwhile, anisotropic media generally carry different spectral-weight distributions along different principal directions.
Equation~\eqref{static_limit_from_kramers_kronig} therefore shows that different directional absorption spectra produce different directional static dielectric responses.
The static limit thus provides a direct link between optical anisotropy in the absorptive channel and dielectric anisotropy in the dispersive channel.

Consequently, the physical interpretation of the Kramers--Kronig relations becomes explicit in the static limit.
The imaginary part supplies the spectral weight through electronic excitations, whereas the real part collects and redistributes that weight into the zero-frequency response.
Within this framework, the complex dielectric function already contains the information required for the subsequent optical quantities.
In the following discussion, we use the same dielectric response to characterize the refractive index, extinction coefficient, absorption coefficient, reflectivity, and energy-loss spectrum.
At this stage, we arrive at the static interpretation of the Kramers--Kronig relations for anisotropic media.
