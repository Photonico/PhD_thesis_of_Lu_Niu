...
% This point also clarifies the practical meaning of the calculated spectra.
% The imaginary part $\varepsilon_2(\omega)$ does not merely reflect the band gap.
% Instead, it collects the combined action of the band dispersion, the occupation pattern, and the direction-resolved transition matrix elements.
% In this sense, $\varepsilon_2(\omega)$ provides a direct bridge between the electronic structure and the optical absorption channel.

% For the present work, we further adopt the long-wavelength limit and evaluate the response within the independent-particle picture built from the ground-state electronic structure.
% Under this approximation, the calculated $\varepsilon_2(\omega)$ describes single-particle interband absorption and resolves the main transition channels that dominate the optical spectra.
% This level of description already captures the principal onset energies, the major absorption peaks, and the directional contrast relevant to the systems studied here.

% Most importantly, the tensor indices $\alpha$ and $\beta$ enter directly through the velocity matrix elements.
% Therefore, the microscopic transition strength already knows the field polarization and the crystal orientation.
% Once the crystal responds differently along different directions, the imaginary part itself becomes anisotropic before we derive any other optical quantity from it.
% We thus arrive at the microscopic picture behind $\varepsilon_2(\omega)$.
% It collects energy-conserving and dipole-allowed interband transitions, and weights them by their direction-resolved transition strength.
% This point naturally leads us to the dielectric response in anisotropic media.
