...
% opening paragraph
In this chapter, we establish the linear optical response framework behind the optical-property calculations used in this thesis.
The optical properties of the materials studied here arise from electrons driven by an external electromagnetic field.
Under the applied field, the electronic system moves away from its static equilibrium and develops an induced polarization $\vec{P}(\vec{r},t)$.
In principle, the full time-dependent many-body response contains the complete optical information of the system.
In practice, the linear-response regime allows us to describe this response through the electric susceptibility tensor $\chi_{\alpha\beta}(\omega)$ and the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$.
We therefore follow the route from Maxwell electrodynamics in matter to the complex dielectric function $\varepsilon(\omega)$, and then to the optical properties derived from it.

The chapter is organized as follows.
In section~\ref{sec:toward_electromagnetic_response_in_matter}, we start from Maxwell equations in matter and introduce polarization, electric displacement, and the dielectric tensor.
In section~\ref{sec:from_electronic_response_to_dielectric_function}, we connect the electronic response to the complex dielectric function and discuss its direction-resolved form in anisotropic media.
In section~\ref{sec:causality_and_kramers_kronig_relations}, we introduce causality and derive the Kramers--Kronig relations for the dielectric response.
Finally, in section~\ref{sec:from_dielectric_function_to_optical_properties}, we use the dielectric function to determine the absorption coefficient $\alpha(\omega)$, refractive index $n(\omega)$, extinction coefficient $k(\omega)$, reflectivity $R(\omega)$, and energy-loss spectrum $L(\omega)$.
...
Therefore, it is convenient to introduce the in-plane dielectric function $\varepsilon_{\varparallel}(\omega)$ and the out-of-plane dielectric function $\varepsilon_{\perp}(\omega)$~\cite{nye1985physical,guo2021complete,matthes2016influence}:
...
Therefore, the energy-loss spectrum $L_{\lambda}(\omega)$ characterizes the energy transferred from an external probe to the medium and more directly identifies the dominant dissipation channels than absorption or reflectivity alone~\cite{niu2024electronic,khan2025insight,huang2023optical,suk2024ultrafast,cheng2025two,tang2025unveiling,leon2026momentum}.
