...
These relations give the positive-frequency form of the dielectric response in the present study.
Once the absorptive part $\varepsilon_{2,\lambda\lambda}(\omega)$ is obtained from the electronic transitions, equation~\eqref{kk_positive_frequency_dielectric_component} determines the dispersive part $\varepsilon_{1,\lambda\lambda}(\omega)$.
The same relation also preserves the causal connection between the two parts of the dielectric response.
...
The imaginary part records the energy absorbed by the medium at a given frequency, whereas the real part records the associated dispersion generated by the same response process.
The static limit then follows directly from equation~\eqref{kk_positive_frequency_dielectric_component}.
We derive its explicit form and physical interpretation in the next subsection.
