...
Here, the kernel object is the polarization field $\vec{P}(\vec{r},t)$.
...
Secondly, we transform equation~\eqref{time_domain_constitutive_relation} to the frequency domain, and arrive at:
...
This equation already demonstrates that the susceptibility already determines the linear electric response of the medium.
Substituting equation~\eqref{frequency_domain_polarization_susceptibility_relation} into equation~\eqref{electric_displacement_field_definition}, we then obtain:
...
This result motivates the definition of the dielectric tensor:

\begin{equation}
    \varepsilon_{\alpha\beta}(\omega)
    \coloneqq\delta_{\alpha\beta}
    + \chi_{\alpha\beta}(\omega)
\label{dielectric_tensor_definition}
\end{equation}

so that equation~\eqref{electric_displacement_from_susceptibility} can be rewritten as
...
Consequently, the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$ provides the most compact description of the linear electric response.
For an isotropic medium, equation~\eqref{frequency_domain_constitutive_relation_with_dielectric_tensor} reduces to a scalar dielectric function $\varepsilon(\omega)$.
For crystalline materials, however, anisotropy generally survives, so the tensor form remains the more natural choice.
This point becomes particularly important when we compare different directions of the optical response in the later calculations~\cite{han2023temperature,lima2022optical}.

Finally, the logical route has now become clear.
We started from the polarization field, introduced the susceptibility tensor, and then arrived at the dielectric tensor.
Therefore, the dielectric function does not enter as an isolated definition.
Instead, it emerges naturally from the constitutive relation that closes Maxwell electrodynamics in matter.
This route also prepares the ground for the next subsection, where we discuss the dielectric function itself in a more explicit form.
