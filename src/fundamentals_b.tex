...
where, $\rho(\vec{r},t)$ and $\vec{J}(\vec{r},t)$ denote the total charge density and total current density, respectively.
...
where, $\chi_{\alpha\beta}(\omega)$ denotes the frequency-dependent electric susceptibility tensor.
...
Therefore, the dielectric tensor emerges naturally from the constitutive relation that closes Maxwell electrodynamics in matter.
Meanwhile, anisotropy makes the directional dependence of this response explicit.

At the same time, the dielectric response is not only directional but also dynamical.
In linear response theory, this dynamical character is constrained by causality:
the polarization at a given time can only depend on the electric field applied at earlier times.
Accordingly, the susceptibility kernel must satisfy
\begin{equation}
    \chi_{\alpha\beta}(t-t')=0
    \qquad \text{for } t<t' .
\label{causal_condition_susceptibility_kernel}
\end{equation}
This causal structure will become important in the next subsection,
where we introduce the complex dielectric function and its frequency dependence more explicitly.
...
we introduced the electric susceptibility tensor $\chi_{\alpha\beta}(\omega)$,
and then arrived at the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$.
This tensor emerges naturally from the constitutive relation that closes Maxwell electrodynamics in matter,
while anisotropy makes the directional dependence of the response explicit.
...
where, $\varepsilon_{1,\alpha\beta}(\omega)$ denotes the real part of the dielectric tensor,
...
where, $\varepsilon_1(\omega)$ denotes the real part of the dielectric function,
...
Here, $\vec{k}$ denotes the crystal momentum in the first Brillouin zone. In the optical regime,
the photon momentum is negligible on the scale of the Brillouin zone, so the transition is effectively vertical in $\vec{k}$-space~\cite{fox2010optical}. 
Accordingly, the transition strength is determined by the velocity operator $\hat{v}_\alpha$ rather than by the band energies alone.
...
where, the real part $\varepsilon_{1,\lambda\lambda}(\omega)$ describes the directional dispersive response,
...
where, $\chi_{\alpha\beta}(\omega)$ denotes the frequency-domain electric susceptibility tensor.
...
where, $\varepsilon_{1,\lambda\lambda}(0)$ characterizes the static dielectric response along the principal direction $\lambda$.
...
where, $n_{\lambda}(\omega)$ denotes the refractive index,
...
The following figure presents the calculated absorption coefficient $\alpha_{\lambda}(\omega)$ spectra of the present systems for the in-plane and out-of-plane directions~\cite{niu2024electronic,khan2025insight,huang2023optical,suk2024ultrafast,cheng2025two,tang2025unveiling,kim2024hyperspectral}:
...
Accordingly, the absorption coefficient $\alpha_{\lambda}(\omega)$ characterizes the rate of optical attenuation in the medium.
...
where, $\mu_{\lambda\lambda}(\omega)$ denotes the magnetic permeability.
...
With these expressions, we present the calculated refractive index $n_{\lambda}(\omega)$ spectra of $\alpha$-Beryllene and $\beta$-Beryllene~\cite{niu2024electronic,khan2025insight,huang2023optical,suk2024ultrafast,cheng2025two,tang2025unveiling,kim2024hyperspectral}:
...
Accordingly, the refractive index $n_{\lambda}(\omega)$ and extinction coefficient $k_{\lambda}(\omega)$ characterize phase propagation and amplitude decay in the medium.
...
where, $r_{\lambda}(\omega)$ denotes the amplitude reflection coefficient,
...
The following figure presents the calculated reflectivity $R_{\lambda}(\omega)$ spectra of $\alpha$-Beryllene and $\beta$-Beryllene for the in-plane and out-of-plane directions~\cite{niu2024electronic,khan2025insight,huang2023optical,suk2024ultrafast,cheng2025two,tang2025unveiling,kim2024hyperspectral}:
...
Accordingly, the reflectivity $R_{\lambda}(\omega)$ characterizes the surface reflection of the medium.
...
The energy-loss spectrum $L_{\lambda}(\omega)$ is directly related to the loss channels measured in electron energy-loss spectroscopy.
...
The following figure presents the calculated energy-loss $L_{\lambda}(\omega)$ spectra of $\alpha$-Beryllene and $\beta$-Beryllene for the in-plane and out-of-plane directions~\cite{niu2024electronic,khan2025insight,huang2023optical,suk2024ultrafast,cheng2025two,tang2025unveiling,kim2024hyperspectral}:
...
Accordingly, the energy-loss spectrum $L_{\lambda}(\omega)$ characterizes the energy transferred from an external probe to the medium and identifies the dominant dissipation channels in the dielectric response.
