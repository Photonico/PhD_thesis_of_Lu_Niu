...
% Next, we rewrite this macroscopic picture in the language of crystal states.
% In a periodic solid, the relevant microscopic process is an interband transition from an occupied valence state $\ket{\psi_{v\vec{k}}}$ to an empty conduction state $\ket{\psi_{c\vec{k}}}$.
...
% Therefore, the transition is effectively vertical in $\vec{k}$-space.
% At the same time, the transition strength depends on the velocity operator $\hat{v}_\alpha$, because the electric field drives electronic motion rather than the band energies alone~\cite{adler1962quantum,wiser1963dielectric}.

% For the independent-particle picture adopted in the present work, the imaginary part of the dielectric tensor therefore takes the standard interband form:
