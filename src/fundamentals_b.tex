...
Firstly, the causal structure in equation~\eqref{frequency_domain_susceptibility_from_causality} and the reality condition in equation~\eqref{reality_condition_response_functions} do not allow the real and imaginary parts of the dielectric response to vary independently.
Instead, they are connected by a Hilbert-transform pair.
For the dielectric tensor, we write the general frequency-domain form as:
...
\begin{equation}
    ...
    \,\dif{\omega'}
\label{kk_real_part_general_dielectric_tensor}
\end{equation}
...
\begin{equation}
    ...
    \,\dif{\omega'}
\label{kk_imaginary_part_general_dielectric_tensor}
\end{equation}

Here, $\mathcal{P}$ denotes the Cauchy principal value.
It removes the singular contribution at $\omega'=\omega$ and keeps the dispersion integral well defined.

These two expressions assign clear physical roles to the two parts of the dielectric response.
The imaginary part $\varepsilon_{2,\alpha\beta}(\omega)$ characterizes the absorptive channel.
The real part $\varepsilon_{1,\alpha\beta}(\omega)$ characterizes the dispersive channel.
Causality then binds these two channels into a single complex response function.

Next, we use the symmetry relations in equation~\eqref{parity_of_dielectric_components} to reduce the full-frequency integrals to positive-frequency integrals.
In the principal-axis representation, each complex dielectric component $\varepsilon_{\lambda\lambda}(\omega)$ satisfies the Kramers--Kronig relations in the form:
...
\begin{equation}
    ...
    1
    +
    \frac{2}{\pi}\mathcal{P}\int_{0}^{\infty}
    \frac{\omega'\varepsilon_{2,\lambda\lambda}(\omega')}{\omega'^2-\omega^2}
    \,\dif{\omega'}
\label{kk_real_part_positive_frequency_dielectric_component}
\end{equation}

\begin{equation}
    \varepsilon_{2,\lambda\lambda}(\omega)
    =
    -\frac{2\omega}{\pi}\mathcal{P}\int_{0}^{\infty}
    \frac{\varepsilon_{1,\lambda\lambda}(\omega')-1}{\omega'^2-\omega^2}
    \,\dif{\omega'}
\label{kk_imaginary_part_positive_frequency_dielectric_component}
\end{equation}

These relations are the working expressions for the dielectric function in the present study.
Once the absorptive part $\varepsilon_{2,\lambda\lambda}(\omega)$ is obtained from the electronic transitions, equation~\eqref{kk_real_part_positive_frequency_dielectric_component} determines the dispersive part $\varepsilon_{1,\lambda\lambda}(\omega)$.
Meanwhile, equation~\eqref{kk_imaginary_part_positive_frequency_dielectric_component} shows the inverse relation and completes the causal structure of the dielectric response.

Consequently, the complex dielectric function carries a unified physical meaning.
The imaginary part records the energy absorbed by the medium at a given frequency, whereas the real part records the accompanying dispersion generated by the same response process.
The static limit follows directly from equation~\eqref{kk_real_part_positive_frequency_dielectric_component}.
We derive its explicit form and physical meaning in the next subsection.
