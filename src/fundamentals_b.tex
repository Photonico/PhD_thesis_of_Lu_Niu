...
These two parts of the dielectric function together determine the optical response~\cite{wooten2013optical, born2013principles}.
...
\begin{figure}[htbp]
    \centering
    % \includegraphics[width=\textwidth]{figures_ch3/3.1_dielectric.pdf}
    \includegraphics[width=0.9\textwidth]{figures_ch3/3.1_dielectric_alt.pdf}
    \caption{Representative real and imaginary parts of the in-plane and out-of-plane dielectric function as a function of photon energy for a two-dimensional heterostructure. The figure illustrates the direction-resolved complex dielectric response discussed in the text.}
...
connect these quantities to the complex dielectric function $\varepsilon(\omega)$~\cite{wooten2013optical}.
...
That imaginary part drives the amplitude decay in the medium~\cite{wooten2013optical, dressel2002electrodynamics,
gajdovs2006linear}.
...
The following figure presents the calculated absorption coefficients of the present systems for the in-plane and out-of-plane directions. % cite my third project here

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.90\textwidth]{figures_ch3/4.1_absroption_alt.pdf}
    \caption{The absorption coefficients for in-plane and out-of-plane as functions of photon energy.}
    \label{absorption_coefficient_present_systems}
\end{figure}

These two components exhibit different onset energies, peak positions, and peak intensities.
This directional variation reflects the anisotropic dielectric response established in the previous section.
...
which characterize the propagation of the electromagnetic wave.
...
Starting from the dielectric response, we derive explicit expressions for $n_{\lambda}(\omega)$ and $k_{\lambda}(\omega)$.

For a monochromatic wave propagating along the principal direction $\lambda$,
we firstly represent the electric field as:
...
while its real and imaginary parts respectively represent $n_{\lambda}(\omega)$ and $k_{\lambda}(\omega)$, which correspond to the refractive index and extinction coefficient.

This expression separates phase evolution from amplitude attenuation.
...
We then write the general relation between the complex refractive index $N_{\lambda}(\omega)$, dielectric function $\varepsilon_{\lambda\lambda}(\omega)$, and magnetic permeability $\mu_{\lambda\lambda}(\omega)$ as:
...
We therefore derive the coupled relations:
...
These two equations relate the refractive index $n_{\lambda}(\omega)$ and the extinction coefficient $k_{\lambda}(\omega)$ to its real part $\varepsilon_{1,\lambda\lambda}(\omega)$ and imaginary part $\varepsilon_{2,\lambda\lambda}(\omega)$.

\# Draft % Draft

These two equations show the physical division of labor directly.
The real part $\varepsilon_{1,\lambda\lambda}(\omega)$ fixes the dispersive balance between phase propagation and damping,
whereas the imaginary part $\varepsilon_{2,\lambda\lambda}(\omega)$ transfers spectral weight into the damping channel.

Solving equation~\eqref{refractive_index_extinction_coefficient_auxiliary_relations},
we obtain the refractive index:

\begin{equation}
    n_{\lambda}(\omega)
    = \left[
        \frac{
            \sqrt{\varepsilon_{1,\lambda\lambda}^{2}(\omega)
            + \varepsilon_{2,\lambda\lambda}^{2}(\omega)}
            + \varepsilon_{1,\lambda\lambda}(\omega)
        }{2}
    \right]^{1/2}
...
For the present xene systems,
we finally reduce the principal-direction index $\lambda$ to the in-plane channel $\varparallel$ and the out-of-plane channel $\perp$.
This reduction allows a direct comparison between the two optical directions.

We first examine the refractive index spectra of $\alpha$-Beryllene and $\beta$-Beryllene:
...
\begin{figure}[htbp]
    ...
    \label{refractive_index_alpha_beta_beryllene}
\end{figure}

In the in-plane channel,
$\beta$-Beryllene exhibits a much larger low-energy refractive index than $\alpha$-Beryllene,
and it also shows a strong dispersive structure below about $2\,\mathrm{eV}$.
Meanwhile, $\alpha$-Beryllene develops a broader enhancement in the mid-energy range around $5$ to $6\,\mathrm{eV}$.
In the out-of-plane channel,
both systems exhibit stronger resonant features,
with especially sharp structures appearing between about $3$ and $6\,\mathrm{eV}$.
At higher photon energies,
the refractive indices in both directions decrease gradually and approach smaller values.
These trends demonstrate that the phase response inherits the anisotropy already established in the dielectric function.

We next examine the extinction coefficient,
which more directly follows the absorptive part of the dielectric response:
...
In the in-plane channel,
$\beta$-Beryllene carries much stronger low-energy damping than $\alpha$-Beryllene.
This contrast explains why the low-energy optical attenuation differs strongly between the two systems.
In the out-of-plane channel,
both systems exhibit sharper oscillatory structures over the intermediate-energy range,
which indicates a more structured absorptive response along the normal direction.
Whenever $k_{\lambda}(\omega)$ becomes small,
the wave mainly accumulates phase and suffers weaker attenuation.
Together with equation~\eqref{absorption_coefficient_from_extinction_coefficient},
these spectra clarify the directional variation of the absorption coefficient discussed in the previous subsection.

Therefore, the refractive index $n_{\lambda}(\omega)$ and extinction coefficient $k_{\lambda}(\omega)$ provide a compact description of phase propagation and amplitude decay in the medium.
They also supply the two optical quantities required by the Fresnel equations at an interface.
At this stage, we arrive at the refractive index and extinction coefficient as the intermediate response quantities that connect the dielectric function to the measurable interface response.
