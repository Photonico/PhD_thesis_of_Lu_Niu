...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.8\textwidth]{figures_ch3/3.1_dielectric_alt.pdf}
...
We now derive the Kramers--Kronig relations for the dielectric function.
...
In the next subsection, we derive the corresponding static expression and clarify its physical meaning in the next subsection.
...
The Kramers--Kronig relations~\eqref{static_limit_from_kramers_kronig} describes a direct directional dependence in the static limit.
...
We start from the absorption coefficient$\alpha_{\lambda}(\omega)$,
which directly characterizes the attenuation rate of incident light in the medium along the principal direction $\lambda$.

For a monochromatic wave propagating through the medium,
we can describe the intensity attenuation as:
...
Here, $I_{\lambda,0}(\omega)$ and $I_{\lambda}(z,\omega)$
denote the incident and transmitted intensities,
respectively, and $z$ is the propagation distance in the
medium.
Therefore, a larger absorption coefficient corresponds to
stronger attenuation and a shorter optical penetration depth.

To express the absorption coefficient in terms of the
dielectric response, we introduce the complex refractive
index along the principal direction $\lambda$:

\begin{equation}
    N_{\lambda}(\omega)
    = n_{\lambda}(\omega) + i k_{\lambda}(\omega)
\label{complex_refractive_index_for_absorption_coefficient}
\end{equation}

Here, $n_{\lambda}(\omega)$ is the refractive index and
$k_{\lambda}(\omega)$ is the extinction coefficient.
The absorption coefficient is then given by:

\begin{equation}
    \alpha_{\lambda}(\omega)
    = \frac{2\omega\,k_{\lambda}(\omega)}{c}
\label{absorption_coefficient_from_extinction_coefficient}
\end{equation}

where $c$ is the speed of light in vacuum.
This expression shows that the absorption coefficient is
directly controlled by the imaginary part of the complex
refractive index, namely the part that describes amplitude
decay in the medium~\cite{dressel2002electrodynamics,
gajdovs2006linear}.
...
$\varepsilon_{\lambda\lambda}(\omega)
= \varepsilon_{1,\lambda\lambda}(\omega)
+ i \varepsilon_{2,\lambda\lambda}(\omega)$,
we obtain the absorption coefficient directly from the
...
Equation~\eqref{absorption_coefficient_from_dielectric_function}
shows that the absorption coefficient is not an independent
response quantity.
Instead, it reorganizes the same dielectric response into a
form that directly measures optical attenuation in real space.
The absorptive part
$\varepsilon_{2,\lambda\lambda}(\omega)$ provides the
spectral weight generated by electronic transitions,
whereas the dispersive part
$\varepsilon_{1,\lambda\lambda}(\omega)$ modifies the
magnitude and energy dependence of the resulting attenuation.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures_ch3/4.1_absroption_alt.pdf}
    \caption{The in-plane and out-of-plane absorption
    coefficients of the present systems as functions of
    photon energy.}
    \label{absorption_coefficient_present_systems}
\end{figure}

Figure~\ref{absorption_coefficient_present_systems} shows the
calculated absorption coefficients for the present systems in
the in-plane and out-of-plane directions.
The two components exhibit different onset energies, peak
positions, and peak intensities.
This behavior directly reflects the anisotropic dielectric
response established in the previous section.
Once the electronic transitions generate different spectral
weights along different directions, the corresponding optical
attenuation also becomes direction dependent.

The low-energy onset of
$\alpha_{\lambda}(\omega)$ marks the beginning of the
optically allowed transitions.
As the photon energy increases, the broader peaks describe
energy ranges where interband transitions contribute more
strongly to the absorption process.
Therefore, the absorption coefficient translates the
directional excitation spectrum of the medium into a direct
measure of directional optical attenuation.

Consequently, the absorption coefficient provides the most
direct description of how strongly the medium attenuates
incident light.
We next turn to the complex refractive index and determine
