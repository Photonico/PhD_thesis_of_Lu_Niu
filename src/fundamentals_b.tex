...
This equation already describes that the susceptibility already determines the linear electric response of the medium.
...
We then get the expression the electric displacement field via the electric susceptibility tensor:
...
This equation directly suggests the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$:
...
Using this dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$,
we can rewrite equation~\eqref{electric_displacement_from_susceptibility} as the frequency-domain constitutive relation:
...
This equation describes that the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$ gives a compact description of the linear electric response.
For an isotropic medium, the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$ degenerates to a scalar dielectric function $\varepsilon(\omega)$.
For crystalline materials, however, anisotropy often survives, so we keep the tensor form.
This choice will later allow us to compare the optical response along different directions~\cite{han2023temperature}.

We have now completed the route needed in this subsection.
We started from the polarization field $\vec{P}(\vec{r},t)$, introduced the electric susceptibility tensor $\chi_{\alpha\beta}(\omega)$, and arrived at the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$.
In this sense, the dielectric function does not enter as an isolated definition.
Instead, it emerges from the constitutive relation that closes Maxwell electrodynamics in matter.
With this preparation, we can now move to the dielectric function itself.
