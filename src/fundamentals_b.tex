...
Instead, they are related through the Kramers--Kronig relations.
...
the electric field $E_{\beta}(\vec{r}\,',t')$:
...
Therefore, causality requires that the medium responds only to earlier external driving fields:
...
It ensures that the polarization depends only on earlier driving fields and excludes.
...
Meanwhile, it also describes the phase delay of the response at the same frequency.

We then express the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$ via the electric susceptibility tensor $\chi_{\alpha\beta}(\omega)$:
...
allows us to describe the frequency-domain susceptibility via causality:
...
Since the time integral starts from $t=0$, the response function remains analytic in the upper half of the complex frequency plane.
...
At this stage, the previous discussion has established the analytic structure of the dielectric response in frequency space.
