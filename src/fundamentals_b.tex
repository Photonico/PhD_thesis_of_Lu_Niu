...
An electronic transition contributes only when the initial state is occupied and the final state is empty.
The transition matrix elements
$\bra{\psi_{c\vec{k}}}\hat{v}_{\alpha}\ket{\psi_{v\vec{k}}}$
and
$\bra{\psi_{v\vec{k}}}\hat{v}_{\beta}\ket{\psi_{c\vec{k}}}$
must remain finite, and this requirement gives the optical selection rule.
Meanwhile, the transition energy $E_{c\vec{k}}-E_{v\vec{k}}$ must match the photon energy $\hbar\omega$.
Therefore, a peak in $\varepsilon_{2,\alpha\beta}(\omega)$ appears when many allowed transitions gather at the same excitation energy and carry strong transition strength.
