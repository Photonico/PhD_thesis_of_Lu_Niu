...
At this stage, we have established a direction-resolved complex dielectric response for anisotropic media.

As each component includes a real part and an imaginary part,
we next turn to the Kramers--Kronig relations for each direction to connect the real and imaginary parts of each directional component.
...
We now identify the constraints on the real and imaginary parts of each directional component.

Here, the major physical constraint is causality.
In a causal medium, the external electric field precedes the induced polarization.
...
the real part and the imaginary part cannot vary independently.
Instead, they determine each other through the Kramers--Kronig relations.
...
To express causality explicitly, 
Firstly, we express the linear relation between
the polarization field $P_{\alpha}(\vec{r},t)$ and
the electric field $E_{\beta}(\vec{r}\,',t')$ in the time domain:
...
Secondly, causality requires that the medium responds only to earlier external driving fields:
...
This condition defines a retarded response kernel.
It ensures that the polarization collects only earlier driving fields and excludes any advanced response.

In the present work,
we consider the optical long-wavelength limit,
where the wave-vector dependence of the dielectric response becomes negligible.
The linear response therefore reduces to a frequency-dependent tensor relation:
...
It characterizes the response amplitude and phase delay for each Cartesian component at angular frequency $\omega$.

Draft % Draft

The dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$ then follows from:
\begin{equation}
\varepsilon_{\alpha\beta}(\omega)
=
\delta_{\alpha\beta}
...
The causal condition in equation~\eqref{causal_condition_susceptibility_kernel} allows us to rewrite the frequency-domain susceptibility as:
\begin{equation}
\chi_{\alpha\beta}(\omega)
=
\int_{0}^{\infty}
...
This representation shows the analytic structure directly.
Because the time integral starts from $t=0$, the response function remains analytic in the upper half of the complex frequency plane.

For a real time-domain response kernel, the susceptibility and dielectric tensor further satisfy:
\begin{equation}
\chi_{\alpha\beta}(-\omega)
=
\chi_{\alpha\beta}^{*}(\omega),
...
Therefore, in the principal-axis representation, the real part $\varepsilon_{1,\lambda\lambda}(\omega)$ is an even function of frequency, whereas the imaginary part $\varepsilon_{2,\lambda\lambda}(\omega)$ is an odd function:
\begin{equation}
\varepsilon_{1,\lambda\lambda}(-\omega)
=
\varepsilon_{1,\lambda\lambda}(\omega),
...
At this stage, we have translated the temporal order of a causal response into the analytic structure of the dielectric function in frequency space.
We next use this structure to derive the Kramers--Kronig relations between the real and imaginary parts of the dielectric response.
