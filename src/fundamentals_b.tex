...
In the previous subsection, we presented the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$ as the compact description of the linear electric response.
...
Firstly, for an isotropic medium, the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$ reduces to a scalar dielectric function $\varepsilon(\omega)$:
...
The isotropic medium does not distinguish one Cartesian direction from another, so the tensor structure no longer carries additional directional information.
...
When we choose the Cartesian axes along the principal directions of the crystal, the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$ takes the principal-axis form:
...
For the present work, this directional dependence plays a central role.
