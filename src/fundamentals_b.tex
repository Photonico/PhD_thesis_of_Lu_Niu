...
% Equation~\eqref{independent_particle_imaginary_dielectric_tensor} shows the microscopic action in a transparent way.
% Firstly, an electronic state can contribute only when the initial state is occupied and the final state is empty.
% Secondly, the transition matrix elements
% $\mel{\psi_{c\vec{k}}}{\hat{v}_\alpha}{\psi_{v\vec{k}}}$
% and
% $\mel{\psi_{v\vec{k}}}{\hat{v}_\beta}{\psi_{c\vec{k}}}$
% must remain finite, which imposes the optical selection rule.
% Thirdly, the transition energy $E_{c\vec{k}}-E_{v\vec{k}}$ must match the photon energy $\hbar\omega$.
% Therefore, a peak in $\varepsilon_2(\omega)$ appears when many allowed transitions accumulate at the same excitation energy and carry large transition strength.
