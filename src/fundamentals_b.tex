...
the refractive index $n(\omega)$, the extinction coefficient $k(\omega)$,
the reflectivity $R(\omega)$, and the energy-loss function $L(\omega)$.
...
and causality then determines the real part $\varepsilon_1(\omega)$.
...
The previous subsection established the microscopic origin of the imaginary part $\varepsilon_{2,\alpha\beta}(\omega)$.
We now retain the same directional structure in the dielectric response of anisotropic media~\cite{capper2019optical,huang2023optical}.

For an anisotropic medium, each principal direction carries its own complex dielectric response.
When we choose the coordinate axes along the principal directions, the dielectric tensor $\varepsilon_{\alpha\beta}(\omega)$ takes the principal-axis form:
...
\begin{equation}
    ...
    \begin{pmatrix}
        ...
        0 & 0 & \varepsilon_{zz}(\omega)
    \end{pmatrix}
\label{principal_axis_complex_dielectric_tensor}
\end{equation}

Here, each diagonal component is itself a complex dielectric function.
For $i=x,y,z$, we write:

\begin{equation}
    \varepsilon_{ii}(\omega)
    =
    \varepsilon_{1,ii}(\omega)
    +
    \mathrm{i}\,\varepsilon_{2,ii}(\omega)
\label{direction_resolved_complex_dielectric_function}
\end{equation}

Here, the real part $\varepsilon_{1,ii}(\omega)$ describes the directional dispersive response, whereas the imaginary part $\varepsilon_{2,ii}(\omega)$ describes the directional absorptive response.
Therefore, anisotropy enters the optical response through the difference among the principal components.

For layered systems, the structural plane and the normal direction organize the dielectric response most naturally~\cite{guo2021complete,sujith2023large}.
In the present work, we therefore denote the dielectric response within the structural plane by the in-plane dielectric function $\varepsilon_{\varparallel}(\omega)$, and the response normal to that plane by the out-of-plane dielectric function $\varepsilon_{\perp}(\omega)$.
This notation keeps the comparison between the planar channel and the perpendicular channel explicit in the later spectra.

Once the dielectric response is resolved into directional channels, the derived optical quantities inherit the same anisotropy~\cite{slavich2024exploring,zhao2020inplane}.
Therefore, the refractive index $n(\omega)$, the extinction coefficient $k(\omega)$, the absorption coefficient $\alpha(\omega)$, the reflectivity $R(\omega)$, and the energy-loss function $L(\omega)$ should all be interpreted component by component.

At this stage, we arrive at a direction-resolved complex dielectric response for anisotropic media.
Each component carries a real part and an imaginary part, and causality links these two parts for each direction separately.
We next establish this link through the Kramers--Kronig relations.
